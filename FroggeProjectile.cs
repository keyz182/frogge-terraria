...
public class FroggeProjectile : ModProjectile
{
    ...
    public override void AI()
    {
        ...
        if (!player.dead && player.HasBuff(ModContent.BuffType<FroggeBuff>()))
        {
        ...
        if (!Main.dedServ)
        {
            Lighting.AddLight(Projectile.Center, Projectile.Opacity * 0.9f, Projectile.Opacity * 0.1f,
    ...
    private void UpdateDash(Player player)
    {
        ...
        foreach (var npc in Main.ActiveNPCs)
        {
            ...
            Projectile.velocity +=
                Vector2.Normalize(npc.Center - Projectile.Center) * DashSpeed; // Fling the projectile towards the npc.
        ...
        else if (AIFadeProgress < FadeInTicks + FullBrightTicks)
        {
            ...
            if (Main.rand.NextBool(6))
            {
                var dust = Dust.NewDustDirect(Projectile.position, Projectile.width, Projectile.height,
                    DustID.PinkFairy, 0f, 0f, 200, default, 0.8f);
